...
mkdumprd()
{
	...
	if ! have_compression_in_dracut_args; then
		...
		# 99squash and 95squash-squashfs as well as the new
		# 95squash-erofs. Explicitly set which image type is required
		# otherwise the requested compression algorithm might not be
		# supported.
	...
	if ! is_fadump_capable; then
		...
		# When FIPS mode is enabled, the fips dracut module needs to
		# use /boot/.vmlinuz-${KERNEL}.hmac to verify the integrity of
		# the kernel.
		#
		# If /boot is on a separate partition, the fips module will
		# mount /boot based on the boot= kernel parameter.
		#
		# If /boot is not on a separate partition, the fips dracut
		# module will link /sysroot/boot to /boot. So we need to mount
		# the root partition to /sysroot beforehand.
